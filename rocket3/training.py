...
def make_minecraft_env():
    ...
    target_block_pattern = f".*({'|'.join(target_block_types)}|log).*"
            ...
            CrossViewTaskCallback(
                reset_configs=[
                    CrossViewTaskConfig(
                        ...
                        re_pattern=target_block_pattern,
                        obj_id=2,
                    ),
                ]
            ),
...
def make_policy(checkpoint_path: str):
    """Create the CUDA policy used by MineStudio's trainer and rollouts."""
    from rocket3.policy import load_rocket3_policy

    return load_rocket3_policy(checkpoint_path).to("cuda")


online_dict = ONLINE_CONFIG
env_generator = make_minecraft_env
policy_generator = make_policy
